    ...
    public class ImgComparator
    {
        ...
        public List<List<ImgHash>> FindDuplicatesWithTollerance(int minSimilarity = 90)
        {
            ...
            foreach (var hash in _hashLib)
            {
                ...
                if(i % 200 == 0)
                {
                    Debug.WriteLine($"Step 3 of 4 {i}/{_hashLib.Count()} Finding dupe files. {duplicatesFound.Count()} duplicate groups");
                }
        ...
        public void AddPicFolderByPath(string folderPath, bool includeSubFolders = false)
        {
            ...
            for(int i = 0 ; i < files.Count(); i++)
            {
                if (i % 200 == 0)
                {
                    Debug.WriteLine($"Step 2 of 4 Adding file {i}/{files.Count()} to compare cache");
                }
                AddPicByPath(files[i]);
            }
        }
